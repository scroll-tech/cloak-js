...
async function main() {
  ...
  const l3Client = createPublicClient({ transport: http(c.l3Endpoint(), l3Opts), rpcSchema: rpcSchema<ScrollRpcSchema>() });
  ...
  console.log(`Initiating withdrawal from ${l3Wallet.account.address} (L3) to ${l2Account.address} (L2)`);
  ...
  const withdrawalReceipt = await l3Client.waitForTransactionReceipt({ hash: withdrawalHash, pollingInterval: 500 });
  console.log(`Withdrawal initiated with transaction hash ${withdrawalHash} and status ${withdrawalReceipt?.status}`);
  ...
  const claimReceipt = await l2Client.waitForTransactionReceipt({ hash: claimHash, pollingInterval: 500 });
  console.log(`Claim completed with transaction hash ${claimHash} and status ${claimReceipt?.status}`);
  ...
  const unwrapReceipt = await l2Client.waitForTransactionReceipt({ hash: unwrapHash, pollingInterval: 500 });
  console.log(`Unwrap ETH completed with transaction hash ${unwrapHash} and status ${unwrapReceipt?.status}`);
}
